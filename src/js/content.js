...
(function() {
  ...
  function addCountryToFavicon(countryCode, flag) {
    ...
    try {
      ...
      function createCombinedFavicon(originalImg) {
        // Clear canvas
        ctx.clearRect(0, 0, 32, 32);
  ...
  async function handleCountryInfo(response) {
    const settings = await browserAPI.storage.sync.get(['faviconFlag']);
